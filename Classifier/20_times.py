...
def kernelPCA(data,percentage=0.535):
    dataMat = np.array(data)  
...
def mutual_mutual(data,label,k=57):#互信息
...
def elasticNet(data,label,alpha =np.array([0.97])):
    enetCV = ElasticNetCV(alphas=alpha,l1_ratio=0.2).fit(data,label)
    enet=ElasticNet(alpha=enetCV.alpha_, l1_ratio=0.091)
    enet.fit(data,label)
    mask = enet.coef_ != 0
    new_data = data[:,mask]
...
def sum(result):# "对列表的数值求和"
    s = 0
    for x in result:
        s += x
    return s
def average(result):#"对列表数据求平均值"
    avg = 0
    avg = sum(result)/(len(result)*1.0) #调用sum函数求和
    return avg
###############################################################################################################3
...
for i in range (1,21):
    i=1;
    data_1,mask=lassodimension(shu,label) #降维方法
    X=data_1
    y=label
    sepscores = []
    ytest=np.ones((1,2))*0.5
    yscore=np.ones((1,2))*0.5
#    cv_clf =  lgb.LGBMClassifier() #分类器 全都是默认参数88.45%
#    cv_clf = RandomForestClassifier() #分类器
#    cv_clf = GradientBoostingClassifier() #分类器
#    cv_clf = GaussianNB() #分类器
#    cv_clf = KNeighborsClassifier() #分类器
#    cv_clf = AdaBoostClassifier() #分类器
#    cv_clf = MLPClassifier() #分类器
    cv_clf = SVC(probability=True,kernel='poly') #分类器 POLY核函数
